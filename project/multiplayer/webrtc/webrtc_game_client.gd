...
func _handle_server_message(message: Variant) -> void:
	...
	"""
	if message.mtype == WebRTCGameServer.MessageType.CONNECTED_TO_GAME_SERVER:
		var result: Result = _handle_connected_to_game_server(message.data)
		_respond_to_server(message, result)
	elif message.mtype == WebRTCGameServer.MessageType.WEBRTC_OFFER:
		var result: Result = _handle_webrtc_offer(message.data)
		_respond_to_server(message, result)
	elif message.mtype == WebRTCGameServer.MessageType.WEBRTC_ANSWER:
		var result: Result = _handle_webrtc_answer(message.data)
		_respond_to_server(message, result)
	elif message.mtype == WebRTCGameServer.MessageType.WEBRTC_CANDIDATE:
		var result: Result = _handle_ice_candidate(message.data)
		_respond_to_server(message, result)
	elif message.mtype == WebRTCGameServer.MessageType.WEBRTC_ADD_PEER:
		var result: Result = await _handle_webrtc_add_peer(message.data)
		_respond_to_server(message, result)
	elif message.mtype == WebRTCGameServer.MessageType.SET_GAME_AUTHORITY_ID:
